...
def rebuild_knowledge_base_by_batch():
    """按批次汇总重新构建知识库"""
    print("="*60)
    print("按批次汇总重新构建知识库")
    print("="*60)
    ...
    # 4. 按批次分组
    print(f"\n4. 按批次分组（每批{MIN(BATCH_SIZE, len(posts))}条）...")
    batches = []
    ...
    for batch_idx, batch in enumerate(batches, 1):
        ...
        for i, post in enumerate(batch, 1):
            ...
            if len(content) > 300:
                content = content[:300] + "..."
            batch_summary += f"【发言{i}】[{post['date'].strftime('%Y-%m-%d %H:%M')}]\n{content}\n\n"
        
        print(f"  汇总内容长度: {len(batch_summary)} 字符")
        ...
        try:
            # 清理内容
            cleaned_content = kb._clean_quoted_content(batch_summary)
            
            # 调用LLM分析
            result = llm_client.analyze_post_content(cleaned_content, batch[0]['date'])
            
            print(" 完成")
            
            # 添加到知识库（以批次第一条的时间作为记录时间）
            batch_id = f"BATCH_{batch_idx}_{batch[0]['date'].strftime('%Y%m%d')}"
            kb.add_post(
                post_num=batch_id,
            ...
            print(f"  分析结果: 标签={result.get('tags', [])}, 关键点={len(result.get('key_points', []))}个")
        ...
        print(f" 批次完成，累计处理 {total_analyzed} 条")
...
if __name__ == "__main__":
    rebuild_knowledge_base_by_batch()