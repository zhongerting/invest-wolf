...
# -*- coding: utf-8 -*-
"""
批量发送：使用NVIDIA API (deepseek-ai/deepseek-v4-flash)
无RPM限制，10条一批
"""
...
def rebuild_knowledge_base_nvidia():
    """使用NVIDIA API的批量发送（无RPM限制）"""
    print("="*60)
    print("批量发送知识库构建（NVIDIA API）")
    print("="*60)
    ...
    # 创建使用NVIDIA配置的LLM客户端（rpm_limit=1000表示基本无限制）
    llm_client = LLMClient(
        api_url=Config.LLM_API_URL_NVIDIA,
        api_key=Config.LLM_API_KEY_NVIDIA,
        model=Config.LLM_MODEL_NVIDIA,
        rpm_limit=1000
    )
    # 禁用自动切换到备用API（避免模型不匹配）
