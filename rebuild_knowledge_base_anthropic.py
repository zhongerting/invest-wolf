...
# -*- coding: utf-8 -*-
"""
真正的批量发送：20条一批，使用备用API (Anthropic)
"""
...
def rebuild_knowledge_base_anthropic():
    """使用Anthropic备用API的批量发送"""
    print("="*60)
    print("批量发送知识库构建（20条一批，Anthropic备用API）")
    print("="*60)
    ...
    # 5. 强制使用备用API (Anthropic)
    print("[5/6] 按批次发送LLM分析（使用Anthropic备用API）...")
    print("-" * 60)
...
只返回JSON，不要其他内容。"""
        ...
        if batch_idx % 5 == 0:
            ...
            print(f"    进度: {total_analyzed}/{len(posts)} | 失败: {failed_count} | {elapsed/60:.1f}分钟")
...
if __name__ == "__main__":
    rebuild_knowledge_base_anthropic()